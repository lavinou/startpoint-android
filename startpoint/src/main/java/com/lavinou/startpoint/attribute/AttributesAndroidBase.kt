...
public fun Attributes(concurrent: Boolean): Attributes =
    if (concurrent) ConcurrentSafeAttributes() else HashMapAttributes()

private abstract class AttributesAndroidBase : Attributes {
    ...
    public final override operator fun contains(key: AttributeKey<*>): Boolean =
        map.containsKey(key)
...
private class ConcurrentSafeAttributes : AttributesAndroidBase() {
...
private class HashMapAttributes : AttributesAndroidBase() {
